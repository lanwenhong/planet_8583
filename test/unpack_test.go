...
func printNonZeroFields(v interface{}) {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if fieldVal.Kind() == reflect.Slice && fieldVal.Type().Elem().Kind() == reflect.Uint8 {
			...
			fmt.Printf("  %s（[]byte）: %s(%q)\n", fieldType.Name, hex.EncodeToString(b), b)
		} else {
			// 非 []byte 字段直接打印
...
func TestUnPack(t *testing.T) {
	...
	printNonZeroFields(ups)
}

func TestTransactionRsp(t *testing.T) {
	ctx := context.WithValue(context.Background(), "trace_id", util.GenXid())
	bcd := "0110303801000e8082000000000000000005550008881546331105022635333039353736373438313235373535363430303132333435363738034400059f36020a31"
